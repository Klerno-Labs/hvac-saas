...
export async function createDepositCheckoutSession(
  ...
  if (!estimate.depositRequired) return { success: false, error: 'No deposit required for this estimate' }
  if (estimate.depositStatus === 'paid') return { success: false, error: 'Deposit already paid' }
  ...
  const checkoutSession = await stripe.checkout.sessions.create(
    {
      ...
      success_url: `${appUrl}/portal/${token}/estimates/${estimateId}?deposit=paid`,
      cancel_url: `${appUrl}/portal/${token}/estimates/${estimateId}`,
      metadata: {
        estimateId: estimate.id,
        organizationId: ctx.organizationId,
        type: 'deposit',
