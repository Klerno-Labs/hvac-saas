...
// All entries lowercased — lookup is always key.toLowerCase()
...
export function diffSummary(
  ...
  const b = before ?? {}
  const a = after ?? {}
  const keys = opts?.fields ?? [...new Set([...Object.keys(b), ...Object.keys(a)])]
  const result: { field: string; from: unknown; to: unknown }[] = []
  for (const key of keys) {
    const fromVal = before !== null ? b[key] : null
    const toVal = after !== null ? a[key] : null
    if (!valuesEqual(fromVal, toVal)) {
      result.push({
        field: key,
        from: isSensitive(key) ? '[REDACTED]' : fromVal,
        to: isSensitive(key) ? '[REDACTED]' : toVal,
      })
    }
  }
  return result
