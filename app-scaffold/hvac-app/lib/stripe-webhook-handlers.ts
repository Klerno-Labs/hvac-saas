...
/**
 * Single source of truth for Stripe webhook routing.
 *
 * `dispatchStripeEvent` is invoked from two places:
 *   1. The inbound webhook route (`app/api/stripe/webhook/route.ts`) on first
 *      verified delivery of an event.
 *   2. The retry/replay scheduler (`lib/stripe-webhook-replay.ts`) when a
 *      previously-failed event is re-attempted.
 *
 * The receive path collapses Stripe redeliveries onto a single row (see
 * `recordInboundEvent`) so the handler is not double-invoked for the same
 * event id. Handlers additionally guard against re-application at the data
 * level (e.g. "skip if invoice already paid") as defense in depth.
 */
