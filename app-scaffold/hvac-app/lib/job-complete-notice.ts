...
export async function sendJobCompleteNotice(
  ...
  try {
    ...
    if (!job) {
      return { sent: false, channels: [], skippedReason: 'job_not_found' }
    }

    if (job.status !== 'completed') {
      return { sent: false, channels: [], skippedReason: 'not_completed' }
    }

    if (job.completionNoticeSentAt != null) {
      return { sent: false, channels: [], skippedReason: 'already_sent' }
    }

    if (job.organization.jobCompletionNoticeEnabled === false) {
      return { sent: false, channels: [], skippedReason: 'disabled' }
    }

    // Find the open invoice with the largest outstanding balance
    const openInvoice = job.invoices
    ...
    // Find or create a CustomerReview for the review link
    ...
    await db.job.update({
      ...
      data: {
        ...
        completionNoticeChannels: channels.join(','),
      },
    })
    ...
    console.error('[job-complete-notice] Unexpected error:', error)
