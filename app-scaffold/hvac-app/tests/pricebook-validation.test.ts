...
describe('createPriceBookItemSchema', () => {
  const validItem = {
    name: 'AC Tune-Up',
    flatPriceCents: 15000,
    optionGroups: [
      { tier: 'good' as const, name: 'Basic', priceCents: 12000 },
      { tier: 'better' as const, name: 'Standard', priceCents: 15000 },
      { tier: 'best' as const, name: 'Premium', priceCents: 20000 },
    ],
  }
  ...
  it('rejects an invalid tier value', () => {
    const result = createPriceBookItemSchema.safeParse({
      ...validItem,
      optionGroups: [{ tier: 'premium', name: 'Top', priceCents: 5000 }],
    })
    expect(result.success).toBe(false)
  })
